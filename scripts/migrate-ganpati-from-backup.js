#!/usr/bin/env node
/**
 * Migrate content/ganpati/*.yaml from backup.
 * Writes only under content/ganpati/.
 *
 *   node scripts/migrate-ganpati-from-backup.js
 */
const fs = require('fs');
...
const { migrateGanpatiDoc } = require('./lib/ganpati-lyrics');

const SECTION = 'ganpati';
...
for (const name of files) {
  ...
  const migrated = migrateGanpatiDoc({ ...backupDoc });
  const strategy = migrated._ganpatiStrategy || 'unknown';
  delete migrated._ganpatiStrategy;
...
console.log(`Migrated ${rows.length} ganpati files → ${outDir}`);
